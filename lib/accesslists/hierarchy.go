...
func (h *hierarchy) GetMembers(accessListName string) ([]*accesslist.AccessListMember, error) {
	return h.getMembers(accessListName, make(map[string]struct{}))
}

func (h *hierarchy) getMembers(accessListName string, seen map[string]struct{}) ([]*accesslist.AccessListMember, error) {
	node, exists := h.Nodes[accessListName]
	if !exists {
		return nil, trace.NotFound("Access List '%s' not found", accessListName)
	}

	// ignore any existing circular references
	...
	for _, memberList := range node.MemberLists {
		members, err := h.getMembers(memberList.AccessList.GetName(), seen)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		allMembers = append(allMembers, members...)
